...
mod test {
    ...
    fn parses() {
        ...
        assert_eq!(
            MX::read(buf.len() as _, &mut Cursor::new(buf)).unwrap(),
            MX {
                preference: 10,
    ...
    fn incorrect_record_length() {
        ...
        assert_eq!(
            MX::read(6, &mut Cursor::new(buf)),
            Err(WireError::WrongLabelLength {
                stated_length: 6,
                length_after_labels: 7
            })
        );
    }
    ...
    #[test]
    fn record_empty() {
        assert_eq!(MX::read(0, &mut Cursor::new(&[])), Err(WireError::IO));
    }
    ...
    fn buffer_ends_abruptly() {
        ...
        assert_eq!(MX::read(23, &mut Cursor::new(buf)), Err(WireError::IO));
    }
}
