...
mod test {
    ...
    fn parses() {
        ...
        assert_eq!(
            EUI48::read(buf.len() as _, &mut Cursor::new(buf)).unwrap(),
            EUI48 {
                octets: [0x00, 0x7F, 0x23, 0x12, 0x34, 0x56]
            }
        );
    }
    ...
    fn record_too_short() {
        ...
        assert_eq!(
            EUI48::read(buf.len() as _, &mut Cursor::new(buf)),
            Err(WireError::WrongRecordLength {
                stated_length: 3,
                mandated_length: MandatedLength::Exactly(6)
            })
        );
    }
    ...
    fn buffer_ends_abruptly() {
        ...
        assert_eq!(EUI48::read(6, &mut Cursor::new(buf)), Err(WireError::IO));
    }
    ...
    fn hex_rep() {
        ...
        assert_eq!(record.formatted_address(), "00-7f-23-12-34-56");
    }
}
