...
mod test {
    ...
    fn parses() {
        ...
        assert_eq!(
            SOA::read(buf.len() as _, &mut Cursor::new(buf)).unwrap(),
            SOA {
    ...
    fn incorrect_record_length() {
        ...
        assert_eq!(
            SOA::read(89, &mut Cursor::new(buf)),
            Err(WireError::WrongLabelLength {
                stated_length: 89,
                length_after_labels: 30
            })
        );
    }
    ...
    #[test]
    fn record_empty() {
        assert_eq!(SOA::read(0, &mut Cursor::new(&[])), Err(WireError::IO));
    }
    ...
    fn buffer_ends_abruptly() {
        ...
        assert_eq!(SOA::read(23, &mut Cursor::new(buf)), Err(WireError::IO));
    }
}
