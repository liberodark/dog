...
impl Wire for NAPTR {
    ...
    fn read(stated_length: u16, c: &mut Cursor<&[u8]>) -> Result<Self, WireError> {
        ...
        let length_after_labels = 2
            + 2
            + 1
            + u16::from(flags_length)
            + 1
            + u16::from(service_length)
            + 1
            + u16::from(regex_length)
            + replacement_length;
...
mod test {
    ...
    fn parses() {
        let buf = &[
            ...
            0x0e, // regex length
            0x5c, 0x64, 0x5c, 0x64, 0x3a, 0x5c, 0x64, 0x5c, 0x64, 0x3a, 0x5c, 0x64, 0x5c,
            0x64, // regex
            0x0b, 0x73, 0x72, 0x76, 0x2d, 0x65, 0x78, 0x61, 0x6d, 0x70, 0x6c, 0x65, 0x06, 0x6c,
            0x6f, 0x6f, 0x6b, 0x75, 0x70, 0x03, 0x64, 0x6f, 0x67, 0x00, // replacement
        ];

        assert_eq!(
            NAPTR::read(buf.len() as _, &mut Cursor::new(buf)).unwrap(),
            NAPTR {
                order: 5,
                preference: 10,
                flags: Box::new(*b"s"),
    ...
    fn incorrect_length() {
        ...
        assert_eq!(
            NAPTR::read(11, &mut Cursor::new(buf)),
            Err(WireError::WrongLabelLength {
                stated_length: 11,
                length_after_labels: 13
            })
        );
    }
    ...
    #[test]
    fn record_empty() {
        assert_eq!(NAPTR::read(0, &mut Cursor::new(&[])), Err(WireError::IO));
    }
    ...
    fn buffer_ends_abruptly() {
        ...
        assert_eq!(NAPTR::read(23, &mut Cursor::new(buf)), Err(WireError::IO));
    }
}
