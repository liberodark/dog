...
impl Wire for EUI64 {
    ...
    fn read(stated_length: u16, c: &mut Cursor<&[u8]>) -> Result<Self, WireError> {
        if stated_length != 8 {
            warn!(
                "Length is incorrect (record length {:?}, but should be eight)",
                stated_length
            );
...
mod test {
    ...
    fn parses() {
        ...
        assert_eq!(
            EUI64::read(buf.len() as _, &mut Cursor::new(buf)).unwrap(),
            EUI64 {
                octets: [0x00, 0x7F, 0x23, 0x12, 0x34, 0x56, 0x78, 0x90]
            }
        );
    }
    ...
    fn record_too_short() {
        ...
        assert_eq!(
            EUI64::read(buf.len() as _, &mut Cursor::new(buf)),
            Err(WireError::WrongRecordLength {
                stated_length: 3,
                mandated_length: MandatedLength::Exactly(8)
            })
        );
    }
    ...
    fn buffer_ends_abruptly() {
        ...
        assert_eq!(EUI64::read(8, &mut Cursor::new(buf)), Err(WireError::IO));
    }
    ...
    fn hex_rep() {
        ...
        assert_eq!(record.formatted_address(), "00-7f-23-12-34-56-78-90");
    }
}
