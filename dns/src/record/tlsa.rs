...
mod test {
    ...
    fn parses() {
        ...
        assert_eq!(
            TLSA::read(buf.len() as _, &mut Cursor::new(buf)).unwrap(),
            TLSA {
                certificate_usage: 3,
                selector: 1,
                matching_type: 1,
    ...
    fn record_too_short() {
        ...
        assert_eq!(
            TLSA::read(buf.len() as _, &mut Cursor::new(buf)),
            Err(WireError::WrongRecordLength {
                stated_length: 3,
                mandated_length: MandatedLength::AtLeast(4)
            })
        );
    }
    ...
    #[test]
    fn record_empty() {
        assert_eq!(TLSA::read(0, &mut Cursor::new(&[])), Err(WireError::IO));
    }
    ...
    fn buffer_ends_abruptly() {
        ...
        assert_eq!(TLSA::read(6, &mut Cursor::new(buf)), Err(WireError::IO));
    }
}
